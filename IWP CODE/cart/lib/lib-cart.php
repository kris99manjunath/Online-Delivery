...
class Cart extends DB {
  ...
  // pAdd () : add new product
    ...
    $sql = "INSERT INTO `products1` (`product_name`, `product_image`, `product_description`, `product_price`) VALUES (?, ?, ?, ?)";
  ...
  // pEdit () : update product
    ...
    $sql = "UPDATE `products1` SET `product_name`=?, `product_image`=?, `product_description`=?, `product_price`=? WHERE `product_id`=?";
  ...
  // pDel () : delete product
    ...
    $sql = "DELETE FROM `products1` WHERE `product_id`=?";
    $cond = [$id];
    return $this->exec($sql, $cond);
  }
