  ...
  case "show":
    ...
    <table >
      ...
      foreach ($_SESSION['cart'] as $id => $qty) {
        ...
        $total += $sub;
        // THE PRODUCT
        printf("<tr><td><input id='qty_%u' onchange='cart.change(%u);' type='number' value='%u'/></td><td>%s</td><td>Rs %0.2f</td></tr>", $id, $id, $qty, $products[$id]['product_name'], $sub
        );
      }
      ?>
      <tr>
    ...
    <?php if ($total > 0) { ?>
    <button type="button"><a href = "thank-you.php">Checkout!</a></button>
    <?php
    }
    break;
